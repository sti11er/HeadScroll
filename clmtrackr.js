...
var previousY;
var i = 1;
var startTime, timeDifference, len, maxLen;
var rate_of_change;
var nodLR=false, nodUD=true;
...
function faceRotation(landmarks)
{
  ...
  if (nodUD){
    ...
    if (i % 3 == 0){
      ...
      if (Math.abs(maxLen - (heightFace / widthFace)) < 0.01){
        if (nodUp == false && nodDown == false)
          maxLen = heightFace / widthFace;
      }
      if (rate_of_change < 0.08 || rate_of_change > - 0.08){
        if (Math.abs(heightFace/widthFace - maxLen) < 0.02 && nodUp == false){
